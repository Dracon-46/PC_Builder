...
public interface IOrderService
{
    Task<Order> PlaceOrderAsync(Build build, string customerName, string customerEmail,
        string customerPhone, string shippingAddress, decimal? finalPrice = null);
...
public class OrderService : IOrderService
{
    ...
    public async Task<Order> PlaceOrderAsync(Build build, string customerName, string customerEmail,
        string customerPhone, string shippingAddress, decimal? finalPrice = null)
    {
        // Se finalPrice for informado (após Decorator de descontos), usa ele; senão usa preço bruto
        var total = finalPrice.HasValue && finalPrice.Value > 0 ? finalPrice.Value : build.TotalPrice;
